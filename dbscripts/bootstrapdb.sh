...
# Script is for Creating Mapping Database Tables. This script should be executed so that all users and roles are created for the DB.
# Based on where you execute this script you will need to get the host DB and power cart credentials for that DB. Any questions contact fimapping@concur.com
# You can also contact devs @slack #fi-mapping
...
if [ "$#" -eq 0 ]; then
	...
	echo "This script creates all schemas as well as mapping, mappingsupport, and sa users to mirror prod, however all passwords are empty by default."
	exit 1
elif [ "$#" -eq 1 ]; then
	username="poweruser";
	echo "Usage: $0 hostname User (User to be logged in as in my Sql Server)"
